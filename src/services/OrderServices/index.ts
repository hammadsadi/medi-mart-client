...
export const  updateSingleOrderCheckingStatusForAdmin = async (orderId:string, checkingStatus:{status:string}) => {
  try {
    const res = await fetch(
...
export const  updateSingleOrderDeliveryStatusForAdmin = async (orderId:string, deliveryStatus:{status:string}) => {
  try {
    const res = await fetch(
